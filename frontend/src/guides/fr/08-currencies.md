...
Si les finances du foyer sont basées sur le JPY, il n'est pas nécessaire de créer des écritures pour les variations de prix des cryptomonnaies. Pour un achat, enregistrez les JPY payés et la quantité acquise ; pour une vente, les JPY reçus et la quantité cédée ; pour un paiement direct en cryptomonnaie, le montant de la dépense et la quantité cédée. Les conversions affichées sont des informations de référence et ne modifient pas le livre.

Vous pouvez détenir le même BTC pour les deux usages. Des comptes séparés gardent les soldes distincts même si le code de devise est identique, et le rapprochement compare chaque compte indépendamment. Les quantités du portefeuille ne peuvent être récupérées dans les comptes d'investissement que lorsque BTC est sélectionné dans l'en-tête ; chaque compte, y compris ceux d'usage quotidien, permet aussi une saisie manuelle. Consultez le guide de Rapprochement des soldes cryptographiques.
